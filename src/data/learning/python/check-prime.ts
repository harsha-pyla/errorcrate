...
export const checkPrimeQuestion: LearningQuestion = {
  id: "check-prime",
  title: "Check if a Number is Prime",
  difficulty: "Easy",
  topic: "python",
  problem: {
    description: "Determine whether a given integer is a prime number. A prime number is a number greater than 1 that cannot be formed by multiplying two smaller natural numbers.",
    input: "7",
    output: "True"
  },
  ...
  commonMistakes: [
    ...
    if n % i == 0:
      ...
      correctCode: `for i in range(2, int(n**0.5) + 1):
    if n % i == 0:
        return False`,
  ...
  visualizerSteps: [
    ...
    {
      codeLine: "for i in range(2, int(n**0.5) + 1):",
      description: "Square root of 7 is ~2.64. Range goes from 2 to 2 (int(2.64) + 1 = 3, so loop stops before 3). Start with i = 2.",
      variables: { n: 7, i: 2 },
      callStack: ["is_prime()"]
    },
    {
    ...
    {
      codeLine: "for i in range(2, int(n**0.5) + 1):",
      description: "Loop finishes because 2 was the only index in our range.",
      variables: { n: 7, i: 2 },
      callStack: ["is_prime()"]
    },
    {
